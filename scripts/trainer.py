...
class Trainer():
    def __init__(self, dataloaders, model, render, optimizer, scheduler, args, param_model=None):
        self.train_dl = dataloaders[0]
        self.valid_dl = dataloaders[1]
        self.test_dl = dataloaders[2]
        self.model = model 
        if args.pred_param:
            self.p_model = param_model
        self.render = render
    ...
    def train(self):
        ...
        for e in iter_:
            ...
            if self.args.save_checkpoint and e % self.args.SAVE_FREQ == 0:
                ...
                if self.args.pred_param:
                    state.update({'p_model':self.p_model.state_dict()})
    ...
    def _run_one_epoch(self, cross_valid=False):
        ...
        for i, batch in enumerate(dataloader):
            ...
            if self.args.pred_param:
                aif_d, btl = self.model(batch['rgb_fd'])
                fds = self.p_model(btl)
            else:
                aif_d = self.model(batch['rgb_fd'])
                fds = batch['output_fd']
            
            aif = batch['aif'] if self.args.gt_aif else torch.clip(aif_d[:, :-1], 0, 1)
            ...
            loss = self.cal_loss(aif, dpt, coarse_aif, recon, batch['output'], cross_valid)
            total_loss += loss
    ...
    def cal_loss(self, aif, dpt, coarse_aif, recon, tar, cross_valid):
    ...
    def visualize(self, vis_idx=None):
        ...
        if self.args.pred_param:
            aif_d, btl =  self.model(vis_data['rgb_fd'].unsqueeze(0).cuda())
            fds = self.p_model(btl)
        else:
            aif_d =  self.model(vis_data['rgb_fd'].unsqueeze(0).cuda())
            fds = vis_data['output_fd']
        
        aif = vis_data['aif'].unsqueeze(0).cuda() if self.args.gt_aif else torch.clip(aif_d[:, :-1], 0, 1)
        ...
        for i in range(FS):
            ...
            gt_wandb = wandb.Image(gt_def, caption=f"GT Defocus, fd={fds[i]}")
        ...
        if not self.args.gt_dpt:
            ...
            recon_dpt_wandb = wandb.Image(recon_dpt * 1e2 , caption='Recon Dpt')
            try:
                gt_dpt = vis_data['dpt'].unsqueeze(-1).squeeze().numpy()
    ...
    def load_checkpoint(self, model_name, orig_model=None):
        ...
        if self.args.pred_param:
            p_state_dict = model_dict['p_model']
            self.p_model.load_state_dict(p_state_dict)
    ...
    def eval_model(self):
        ...
        for i, batch in enumerate(tqdm(dataloader)):
            ...
            if not self.args.gt_dpt:
                ...
                dpt_gt = batch['dpt']
                #dpt_gt = torch.clamp(dpt_gt, 1e-10, float("inf"))
        ...
        df_model.to_csv(os.path.join(self.eval_savepath, 'eval.csv'))
